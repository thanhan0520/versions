...
void Game::render()
{
    ...
    // Lấy thông tin từ View hiện tại của cửa sổ
    sf::Vector2f viewPos = window.getView().getCenter();
    sf::Vector2f viewSize = window.getView().getSize();

    // --- VẼ UI COOLDOWN SKILLS ---
    float uiStartX = viewPos.x - 100; // Khai báo lần đầu và duy nhất
    float uiStartY = viewPos.y + (viewSize.y / 2) - 60;

    std::vector<float> cooldowns = {
        player.getSkillQ_Cooldown(), // Bạn cần thêm các hàm get này vào Player.h
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // Nếu đang hồi chiêu thì màu xám, hồi xong thì màu xanh
        if (cooldowns[i] > 0) {
